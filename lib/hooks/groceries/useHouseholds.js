...
export function useHouseholds() {
    ...
    const joinMutation = useMutation({
        ...
        onSuccess: (joinedHousehold) => {
            ...
            localStorage.setItem("activeHouseholdId", joinedHousehold.id);
        },
    });

    // State for active selection
    const [activeHouseholdId, setActiveHouseholdId] = useState(() => {
        // Optimistic Init: Read from local storage immediately to avoid waterfall
    ...
    useEffect(() => {
        // Fallback: If no local ID, or local ID is invalid (not in list), default to first
        if (households?.length > 0) {
            const isValid = households.some(h => h.id === activeHouseholdId);
            if (!isValid) {
                const firstId = households[0].id;
                setActiveHouseholdId(firstId);
                localStorage.setItem("activeHouseholdId", firstId);
            }
        }
